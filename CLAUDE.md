...
>
> **想 30 秒抓回整套架構**：[`docs/project-echo/architecture-and-usage.md`](docs/project-echo/architecture-and-usage.md)（地圖 + 9 層防線 + 操作員流程 + 故障排查 + 已知欠缺）
>
...
- **compose_mode 的 scheduled_post 永不 auto-send**：即使 pre_approved=true + global=false，LLM 草稿仍強制走 review。這條不能繞。
...
2. **Codex 訂閱**（[`app/ai/codex_compose.py`](app/ai/codex_compose.py)，**production-ready, dry-run gated**，2026-04-29 上線）：
   ...
   - 試點社群：**openchat_004（翊／水月觀音道場）**，voice_profile 已從 stub 升級成完整版
...
| 模式 | 觸發者 | 對應 MCP tools |
|---|---|---|
| **小編發文模式（直送）** | 你（push from Lark） | `compose_and_send`、`approve_review`、`add_scheduled_post`（直送 text） |
| **小編發文模式（LLM compose）** | 你（push from Lark） | `add_scheduled_post(brief=..., compose_mode=true, recurrence="weekly:mon@20:00")`，daemon 在 send_at 前 N 小時跑 codex_compose，Lark 卡待審 |
...
**compose_mode 排程貼文（2026-04-30 上線）**：給 brief（主題）而非 text，daemon 在 `send_at - compose_lead_seconds`（預設 4h）觸發 `codex_compose`，產出帶 voice_profile 人格的草稿，推 Lark 卡片等審。雙閘 opt-in（同 watcher）：`ECHO_COMPOSE_BACKEND=codex` env + `community.llm_compose_enabled=true`。recurrence schema 支援 `daily@HH:MM` / `weekly:mon@HH:MM` / `monthly:N@HH:MM`，sent 後自動 spawn 下一次 occurrence。HIL 不變：compose_mode 永不 auto-send。
...
4. **set_operator_nickname(community_id, nickname="...")** ← 最關鍵的一步
     ...
     **嚴禁 AI 從截圖讀字替操作員填值**——這是身份事實，AI 沒有資格替
     使用者代答「你叫什麼名字」。Onboarding 時必須由操作員親口說：
     在 Lark 訊息或 CLI 明確聲明「我在 openchat_NNN 叫 X」，bot 才寫
     進 yaml。歷史：openchat_004 因 onboarding 我看截圖把「翊」誤填
     「妍」，自主鏈基底錯位 6 個月，污染 fingerprints / lifecycle /
     kpi / relationship_graph / review_store（2026-04-30 修復；詳見
     change-log.md 同日 entry）。

     **若操作員提供值，bot 仍必須驗證再寫入**：
     - 用 emulator navigate 進群 → 點群標題 → 群封面頁底部 user-area
       顯示「<頭像>「<我的暱稱>」」，截圖回 Lark 給操作員確認
     - 或更深：點 ≡ menu → 設定 → 個人檔案
     - 寫入後 `set_operator_nickname` 自動回傳 `verification_hint`
       提醒操作員親眼確認

     **不要靠 chat_export 的「以「X」加入聊天」推斷**——LINE 不一定有
     這行。也**不要靠 export 命中數判斷對錯**——fan 圈群 / broadcast
     群操作員本來就少發言，0 hit ≠ nickname 錯。

     **形似漢字陷阱**（人眼看到任一個必須放大或請操作員口述確認）：
     翊/妍/玕/玟/玥、彥/彦/顏、徐/徒/徙、璿/瑢/璇、蓁/榛/溱

     沒這個 bot 永遠不知道哪則訊息是操作員自己的，自主回覆會崩壞。
